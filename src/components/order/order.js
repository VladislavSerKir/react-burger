...
export const Order = ({ orderInfo, ingredients }) => {
    ...
    console.log(returnIngredients(), orderInfo.ingredients);

    return (
        <li className={`pl-6 pr-5 mr-2 ${orderStyles.order}`}>
                ...
                <ul className={` ${orderStyles.orderIngredients}`}>
                    {
                        returnIngredients().map((ingredient, index) => {
                            ...
                            if (index > 5) {
                                ...
                                return <li key={uuidv4()} style={{ zIndex: 6 - index }} className={`${orderStyles.orderIngredientElement}`} >
                                    <img src={ingredient.image} alt={ingredient.name} style={{ zIndex: 6 - index, opacity: 0.4 }} className={`${orderStyles.orderIngredientImage}`} />
                                    <p className={`text  ${orderStyles.orderIngredientCounter}`} style={{ zIndex: 6 }}>{`+${returnIngredients().length - index}`}</p>
                                </li>
                            } else {
                                return <li key={uuidv4()} style={{ zIndex: 6 - index }} className={`${orderStyles.orderIngredientElement}`} >
                                    <img src={ingredient.image} alt={ingredient.name} style={{ zIndex: 6 - index }} className={`${orderStyles.orderIngredientImage}`} />
                                </li>
                            }
